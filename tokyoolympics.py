...
Original file is located at
    https://colab.research.google.com/drive/17sAbjFxVN7fqwZ3tnPx-ooph4cYlpRH_
"""
...
order by NOC, Discipline''').show(25)

pd_athletes_query = spark.sql('''select first(NOC) as NOC, first(Discipline) as Discipline,
count(*) as Count from athletes group by NOC, Discipline
order by NOC, Discipline''').toPandas()

fig = px.bar(pd_athletes_query, x='NOC', y='Count', color='Discipline', title='Number of Athletes from Each Country')
fig.show()
...
fig.update_layout(showlegend=False, height=10000, width=800, title_text='Distribution Gender amonhst each game')
